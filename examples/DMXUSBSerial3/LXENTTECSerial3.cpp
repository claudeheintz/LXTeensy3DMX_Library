/* LXENTTECSerial3.cpp
   ...
   See LXTeensy3DMX.h for license
	...
	LXENTTECSerial3 implements exchanging DMX USB Pro Widget API packets with Arduino
	over the Teensy's Serial3 connection.  LXENTTECSerial3 does not implement the full
...
//
//  Important:  must call Serial3.begin(<baud>) before calling this method
...
void LXENTTECSerial::writeInfo( uint16_t length ) {
	...
	Serial3.write(header,9);
	header[0] = 0x00;
	for(uint16_t n=0; n<length; n++) {
	   Serial3.write(header, 1);				// does not save user data, write as zeros
	}
	header[0] = 0xE7;
	Serial3.write(header,1);
}
